  ...
  descriptor: {
    ...
    lock: {
      value: function (waitTime) {
        waitTime = waitTime || (this.actions.length + 1) * 10;
        locker.lock(paths.lockFilePath(this.dirName), {wait: waitTime, retryWait: 5});
      }
    },
    unlock: {
      value: function () { //make sure to unlock the collection
        return locker.unlock(paths.lockFilePath(this.dirName));
      }
    },
    flush: {
      value: function () {
        ...
        return sequence(jobs).ensure(this.unlock.bind(this));
      }
    }
  }
});